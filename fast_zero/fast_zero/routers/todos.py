...
currentuser_depends = Annotated[User, Depends(get_current_user)]


@router.post('/', response_model=TodoPublic)
def create_todo(todo: TodoSchema, user: currentuser_depends, session: session_depends):
    time = datetime.now()
    db_todo = Todo(
        title=todo.title,
...
def list_todos(session: session_depends, user: currentuser_depends, todo_filter: Annotated[FilterTodo, Query()]):
...
def patch_todo(todo_id: int, session: session_depends, user: currentuser_depends, todo: TodoUpdate):
...
def delete_todo(todo_id: int, session: session_depends, user: currentuser_depends):
